...
/* ================================================================== */
/*  cloud_query_text — întrebare scrisă din web (endpoint /text)      */
/* ================================================================== */
void cloud_query_text(const char *text)
{
    ...
    /* buffer de răspuns — LIPSEA: fără el nu avem unde colecta audio-ul */
    http_buf_t resp = {.buf = NULL, .len = 0, .capacity = 0};
    resp.buf = heap_caps_malloc(65536, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (resp.buf)
        resp.capacity = 65536;
    ...
    esp_http_client_config_t cfg = {
        ...
        .method = HTTP_METHOD_POST,
        .event_handler = http_event_handler,
        .user_data = &resp, /* LIPSEA: fără el, handlerul iese la if(!rb) */
        .timeout_ms = CLOUD_HTTP_TIMEOUT,
        .buffer_size = 2048,
    };
    ...
    if (err == ESP_OK && status == 200 && resp.len > sizeof(wav_header_t))
    {
        /* redare — LIPSEA: era doar un comentariu-stub */
        ui_speaking();
        const int16_t *pcm = (const int16_t *)(resp.buf + sizeof(wav_header_t));
        size_t total_samples = (resp.len - sizeof(wav_header_t)) / sizeof(int16_t);
        const size_t CHUNK_SAMPLES = 2048;
        size_t sent = 0;
        while (sent < total_samples)
        {
        ...
        ui_idle();
        web_push_conversation(text, s_resp_reply); /* întrebarea scrisă + răspunsul */
    }
    else
    {
        ESP_LOGE(TAG, "cloud_query_text: %s, status %d", esp_err_to_name(err), status);
        ui_error();
    }
    heap_caps_free(resp.buf);
    s_cloud_busy = false;
